/*
  Warnings:

  - The values [Send,Request] on the enum `Type` will be removed. If these variants are still used in the database, this will fail.
  - Added the required column `requestType` to the `requests` table without a default value. This is not possible if the table is not empty.

*/
-- CreateEnum
CREATE TYPE "RequestType" AS ENUM ('PAYMENT', 'REFUND', 'OTHER');

-- AlterEnum
BEGIN;
CREATE TYPE "Type_new" AS ENUM ('SEND', 'REQUEST');
ALTER TABLE "transactions" ALTER COLUMN "type" TYPE "Type_new" USING ("type"::text::"Type_new");
ALTER TABLE "requests" ALTER COLUMN "type" TYPE "Type_new" USING ("type"::text::"Type_new");
ALTER TYPE "Type" RENAME TO "Type_old";
ALTER TYPE "Type_new" RENAME TO "Type";
DROP TYPE "Type_old";
COMMIT;

-- AlterTable
ALTER TABLE "requests" ADD COLUMN     "requestType" "RequestType" NOT NULL;
